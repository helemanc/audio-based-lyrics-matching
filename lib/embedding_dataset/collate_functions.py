...
def collate_embeddings_fixed_length(
    ...
    if use_avg_pooling:
        ...
        if is_fixed_shape:
            ...
            stride = int(chunk_size * (1 - overlap_percentage))
            
            # Calculate chunks per song
            all_chunks_per_song = []
            for item in batch:
                for j in range(n_per_class):
                    emb = item[2 + j * 2]
                    if emb is None:
                        all_chunks_per_song.append(1)
                    else:
                        seq_len = emb.shape[0]
                        if seq_len <= chunk_size:
                            all_chunks_per_song.append(1)
                        else:
                            num_chunks = 1 + (seq_len - chunk_size + stride - 1) // stride
                            all_chunks_per_song.append(num_chunks)
            
            max_chunks = max(all_chunks_per_song)
            total_songs = batch_size * n_per_class
            
            # Pre-allocate
            clique_ids = torch.empty(batch_size, dtype=torch.long)
            version_ids = torch.empty(total_songs, dtype=torch.long)
            embeddings = torch.zeros(total_songs, max_chunks, chunk_size, embed_dim)
            masks = torch.zeros(total_songs, max_chunks, chunk_size, dtype=torch.bool)
            ...
            for i, item in enumerate(batch):
                ...
                for j in range(n_per_class):
                    version_ids[song_idx] = item[1 + j * 2]
                    emb = item[2 + j * 2]
                    ...
                    if emb is None:
                        ...
                        seq_len = emb.shape[0]
                        num_chunks = all_chunks_per_song[song_idx]
                        ...
                        for chunk_idx in range(num_chunks):
                            ...
                            embeddings[song_idx, chunk_idx, :chunk_len] = emb[start:end]
                            masks[song_idx, chunk_idx, chunk_len:] = True
                    
                    song_idx += 1
            
            chunk_info = {
                'num_chunks_per_song': all_chunks_per_song,
...
def create_collate_fn(
    ...
    if is_wealy_clews:
        ...
        def wealy_clews_collate_fn(batch: List[Any]) -> Union[List[Dict], List[torch.Tensor]]:
            """
            ...
            for j in range(n_per_class):
                ...
                full_clews_embeddings = torch.zeros(batch_size, full_clews_shape[0], full_clews_shape[1])
                ...
                output.extend([version_ids, wealy_embeddings, full_clews_embeddings,
                             avg_clews_embeddings, clews_masks])
        ...
        def whisper_clews_collate_fn(batch: List[Any]) -> List[torch.Tensor]:
            ...
            if apply_masks_with_padding:
                ...
                for item in batch:
                    for j in range(n_per_class):
                        ...
                        all_multimodal_data.append((multimodal_dict, valid_whisper, valid_clews))
            ...
            for j in range(n_per_class):
                version_ids = torch.empty(batch_size, dtype=torch.long)
                whisper_embeddings = torch.zeros(batch_size, actual_whisper_len, whisper_shape[1])
                whisper_masks = torch.zeros(batch_size, actual_whisper_len, dtype=torch.bool)
                full_clews_embeddings = torch.zeros(batch_size, actual_clews_len, full_clews_shape[1])
                avg_clews_embeddings = torch.zeros(batch_size, avg_clews_dim)
                clews_masks = torch.zeros(batch_size, actual_clews_len, dtype=torch.bool)
            ...
            for i, item in enumerate(batch):
                ...
                for j in range(n_per_class):
                    ...
                    if apply_masks_with_padding:
                        multimodal_dict, valid_whisper, valid_clews = all_multimodal_data[data_idx]
                        data_idx += 1
                        ...
                        if valid_clews > 0:
                            ...
                            padded_clews = torch.zeros(actual_clews_len, full_clews.shape[1])
