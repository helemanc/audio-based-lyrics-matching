...
import gc
from evaluation.eval import compute_baseline
import os 
import re

class TFIDFEvaluator:
    """
    TF-IDF implementation for cover song detection using transcriptions as lyrics
    Supporting both cosine similarity and Lucene-style approaches
    """
    def __init__(self, max_query_terms=12, min_term_freq=1, min_doc_freq=2):
    ...
    def fit_corpus(self, transcriptions: List[str]):
        ...
        self.corpus = [self.preprocess_transcription_as_lyrics(text) for text in transcriptions]
    ...
    def create_query_vector_from_terms(self, selected_terms: List[Tuple[str, float]], 
    ...
    def compute_cover_song_similarities_cosine(self, transcriptions: List[str], 
                                         top_k: int = 100, 
        ...
        """
        print(f"Computing cover song similarities (COSINE) for {len(transcriptions)} transcriptions...")
        ...
        for query_idx in tqdm(range(n), desc="Processing queries"):
            ...
            candidate_similarities = cosine_similarity(query_vector, tfidf_matrix).flatten()
            
            # Convert to torch tensor
            similarities_all[query_idx, :] = torch.from_numpy(candidate_similarities).float()
        ...
        if top_k < n:
            ...
            for query_idx in range(n):
                if valid_mask[query_idx]:
                    ...
                    _, top_indices = torch.topk(similarities_top_k[query_idx, :], k=min(top_k, n-1))
    ...
    def compute_cover_song_similarities_lucene_correct(self, transcriptions: List[str], 
                                                top_k: int = 100,
                                                compute_all: bool = True) -> Dict[str, torch.Tensor]:
        """
        ...
        Returns:
            Dictionary with 'top_k' and optionally 'all' similarity matrices
        """
        print(f"Computing paper-correct Lucene similarities for {len(transcriptions)} transcriptions...")
        ...
        valid_mask = torch.tensor([
            self.tfidf_matrix[i].nnz > 0 for i in range(n)
        ], dtype=torch.bool)
        ...
        for query_idx in tqdm(range(n), desc="Processing queries"):
            ...
            # Convert to torch tensor
            similarities_all[query_idx, :] = torch.from_numpy(query_similarities).float()
    ...
    def compute_tfidf_baseline_cover_song_detection(self, transcriptions: List[str],
                                              clique_ids: torch.Tensor,
                                              version_ids: torch.Tensor,
                                              method: str = 'cosine',
                                              top_k: int = 100,
                                              compute_all: bool = True) -> Dict[str, Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]:
        """
        ...
        """
        print(f"Computing TF-IDF baseline for cover song detection (method: {method.upper()})...")
        
        # Compute similarities based on method
        if method.lower() == 'cosine':
            similarities_dict = self.compute_cover_song_similarities_cosine(transcriptions, top_k, compute_all)
        elif method.lower() == 'lucene':
            similarities_dict = self.compute_cover_song_similarities_lucene_correct(transcriptions, top_k, compute_all)
        else:
        ...
        for key, similarities in similarities_dict.items():
            ...
            print(f"Computing cover song detection metrics ({method.upper()}, {key})...")
...
class BaselinesEvaluator:
    """
    Updated evaluator with both TF-IDF approaches for cover song detection
    """
    def __init__(self, model_name='all-MiniLM-L6-v2', device=None, batch_size=32):
        """Initialize with a SBERT model and TF-IDF evaluator"""
    ...
    def encode_in_chunks(self, texts: List[str], chunk_size: int = 1000) -> torch.Tensor:
    ...
    def compute_similarities_chunked(self, query_embeddings: torch.Tensor, 
    ...
    def convert_similarities_to_distances_chunked(self, similarities: torch.Tensor, 
        ...
        # Process in chunks
        for start_idx in tqdm(range(0, num_rows, chunk_size), desc="Converting to distances"):
    ...
    def compute_ideal_baseline_distances(self, clique_ids: torch.Tensor, 
                                       chunk_size: int = 1000) -> torch.Tensor:
    ...
    def compute_modified_random_baseline_distances(self, clique_ids: torch.Tensor,
        ...
        for i in tqdm(range(0, n, chunk_size), desc="Modified-random (upper-tri)"):
            ...
            for j in range(i, n, chunk_size):
                ...
                clique_matches = clique_ids[i:i_end, None] == clique_ids[None, j:j_end]   # (bi, bj)
    ...
    def evaluate_dataset(self, dataloader,
                ...
                compute_all_tfidf: bool = True) -> Dict[str, Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]:
        """
        ...
        if hasattr(dataset, 'df') and not dataset.df.empty:
            ...
            if valid_transcription_col in dataset.df.columns:
                ...
                print(f"Enhanced validation rate: {valid_transcription_mask.sum().item()}/{len(valid_transcription_mask)} = {valid_transcription_mask.sum().item()/len(valid_transcription_mask)*100:.2f}%")
            else:
                # Fallback to simple empty check if enhanced validation not available
                print(f"Warning: Enhanced validation column '{valid_transcription_col}' not found, using simple empty check")
                valid_transcription_mask = torch.tensor([
            ...
            for batch in tqdm(dataloader, desc="Loading data"):
                clique_batch, version_batch, _, _, _, transcriptions, valid_batch = batch
        ...
        print(f"Valid transcriptions (for modified ideal): {valid_transcription_mask.sum().item()}")
        print(f"Invalid transcriptions (for modified ideal): {(~valid_transcription_mask).sum().item()}")
        print(f"Unique cliques: {len(torch.unique(clique_ids))}")
        
        results = {}
        
        # Compute TF-IDF baselines for cover song detection
        if 'tfidf' in compute_baselines or 'tfidf-cosine' in compute_baselines or 'tfidf-lucene' in compute_baselines:
            ...
            for method in methods_to_compute:
                ...
                method_results = self.tfidf_evaluator.compute_tfidf_baseline_cover_song_detection(
                    all_transcriptions, clique_ids, version_ids, 
                    method=method, top_k=tfidf_top_k, compute_all=compute_all_tfidf
                )
        ...
        if 'random' in compute_baselines:
            ...
            random_distances = self.compute_random_baseline_distances(len(clique_ids), len(clique_ids))
