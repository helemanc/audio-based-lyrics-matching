...
class MeanPool(torch.nn.Module):
    """Mean pooling layer that handles masked sequences"""
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            ...
            pooled = x.sum(dim=1) / (mask.sum(dim=1) + 1e-8)  # (B, C)
        else:
...
class ConvBlock(torch.nn.Module):
    """
    1D Convolutional block with ReLU activation and LayerNorm
    Supports custom stride for temporal downsampling
    """
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1):
        ...
        self.conv = torch.nn.Conv1d(
            ...
            stride=stride,  # Now supports custom stride
    ...
    def forward(self, x):
        # x: (B, C, T)
        x = self.conv(x)  # (B, out_channels, T // stride)
        x = self.relu(x)
        # Transpose for LayerNorm: (B, T // stride, C)
        x = x.transpose(1, 2)
        x = self.norm(x)
        # Transpose back: (B, C, T // stride)
        x = x.transpose(1, 2)
        return x
