...
class CLEWSLoss(nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            anchors_with_pos = has_pos.float().mean()  # fraction of anchors with ≥1 positive
            
            # Mean distances for positives and negatives
            v_dpos = tops.mmean(d, mask=pos_mask) if n_pos_pairs > 0 else \
                     torch.tensor(0.0, device=z.device)
            v_dneg = tops.mmean(d, mask=neg_mask) if n_neg_pairs > 0 else \
                     torch.tensor(0.0, device=z.device)
