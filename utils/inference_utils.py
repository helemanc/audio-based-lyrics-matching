...
def print_configuration(args: Any, fabric: Fabric) -> None:
    ...
    print("=" * 70)
    print(
        f"Mode: {'Overlapping chunks' if args.use_overlapping_chunks else 'Standard'}"
    )
    ...
    print(
        f"Checkpointing: {'Enabled' if not args.disable_checkpointing else 'Disabled'}"
    )
    print(
        f"Memory logging: {'Enabled' if not args.disable_memory_logging else 'Disabled'}"
    )
...
class EvaluationCheckpoint:
    ...
    def _cleanup_old_checkpoints(self, prefix: str) -> None:
        """
        Remove old checkpoint files for current rank with given prefix.

        Args:
            prefix: Checkpoint file prefix (e.g., 'extraction_checkpoint' or 'evaluation_checkpoint')
        """
        try:
            pattern = f"{prefix}_rank_{self.fabric.global_rank}"
            old_files = [
                f
            ...
            print(
                f"[GPU {self.fabric.global_rank}] Warning: failed to cleanup old checkpoints: {e}"
            )

    def save_extraction_checkpoint(
        self, all_data: Dict[str, List], batch_idx: int, args: Any
    ) -> None:
        """
        ...
        # Remove old checkpoints for this rank before saving new one
        self._cleanup_old_checkpoints("extraction_checkpoint")
        ...
        try:
            ...
            print(
                f"[GPU {self.fabric.global_rank}] Saved extraction checkpoint: batch {batch_idx}"
            )
        except Exception as e:
            print(
                f"[GPU {self.fabric.global_rank}] Failed to save extraction checkpoint: {e}"
            )
    ...
    def load_extraction_checkpoint(self) -> Optional[CheckpointData]:
        ...
        latest_file = max(
            checkpoint_files, key=lambda x: int(x.split("_batch_")[1].split(".pkl")[0])
        )
        ...
        try:
            ...
            print(
                f"[GPU {self.fabric.global_rank}] Loaded extraction checkpoint: {latest_file}"
            )
            return checkpoint_data
        except Exception as e:
            print(
                f"[GPU {self.fabric.global_rank}] Failed to load extraction checkpoint: {e}"
            )
            return None
    ...
    def save_evaluation_checkpoint(
        ...
        # Remove old checkpoints for this rank before saving new one
        self._cleanup_old_checkpoints("evaluation_checkpoint")
        ...
        try:
            with open(checkpoint_path, "wb") as f:
                pickle.dump(checkpoint_data, f)
            print(
                f"[GPU {self.fabric.global_rank}] Saved evaluation checkpoint: query {query_idx}"
            )
        except Exception as e:
            print(
                f"[GPU {self.fabric.global_rank}] Failed to save evaluation checkpoint: {e}"
            )
    ...
    def load_evaluation_checkpoint(self) -> Optional[CheckpointData]:
        ...
        latest_file = max(
            checkpoint_files, key=lambda x: int(x.split("_query_")[1].split(".pkl")[0])
        )
        checkpoint_path = os.path.join(self.checkpoint_dir, latest_file)
        ...
        try:
            with open(checkpoint_path, "rb") as f:
                checkpoint_data = pickle.load(f)
            print(
                f"[GPU {self.fabric.global_rank}] Loaded evaluation checkpoint: {latest_file}"
            )
            return checkpoint_data
        except Exception as e:
            print(
                f"[GPU {self.fabric.global_rank}] Failed to load evaluation checkpoint: {e}"
            )
            return None
...
def log_memory_usage(fabric: Fabric, context: str = "", verbose: bool = True) -> None:
    """
...
def safe_all_gather_with_timeout(
    fabric: Fabric, tensor: torch.Tensor, timeout_seconds: int = 300
    ...
    try:
        ...
        print(
            f"[GPU {fabric.global_rank}] All-gather timed out after {timeout_seconds}s"
        )
        signal.alarm(0)
        return None
    except Exception as e:
...
def load_model_and_config(
    args: Any, fabric: Fabric, verbose_memory: bool = True
...
def setup_dataloader(
    conf: DictConfig, args: Any, fabric: Fabric, verbose_memory: bool = True
) -> DataLoader:
    """
...
def extract_embeddings_with_checkpointing(
    ...
    torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, Optional[List[ChunkType]]
]:
    """
    ...
    use_avg = bool(
        getattr(args, "use_avg_pooling", False)
        or (conf is not None and getattr(conf.data, "use_avg_pooling", False))
    )

    # Load or initialize data accumulator
    checkpoint_data = (
        checkpoint_manager.load_extraction_checkpoint()
    ...
    for batch_idx, batch in pbar:
        ...
        try:
            ...
            if args.use_overlapping_chunks and not use_avg:
                ...
                pbar.set_postfix(
                    {"chunks": len(all_data["chunks"]), "songs": unique_songs}
                )
...
def gather_chunks_safely(
    fabric: Fabric, local_chunks: List[ChunkType], verbose_memory: bool = True
) -> List[ChunkType]:
    """
    ...
    try:
        ...
        # All-gather sizes
        all_sizes = safe_all_gather_with_timeout(
            fabric, size_tensor, timeout_seconds=120
        )
        if all_sizes is None:
            print(
                f"[GPU {fabric.global_rank}] Size gather failed; returning LOCAL chunks only"
            )
        ...
        if local_size > 0:
            ...
            local_buf = torch.from_numpy(local_buf_cpu).to(
        ...
        if local_buf.numel() < max_size:
            pad = torch.zeros(
                max_size - local_buf.numel(), dtype=torch.uint8, device=fabric.device
            )
        ...
        all_padded = safe_all_gather_with_timeout(
            fabric, local_buf, timeout_seconds=600
        )
        if all_padded is None:
            print(
                f"[GPU {fabric.global_rank}] Byte-buffer gather failed; returning LOCAL chunks only"
            )
            return local_chunks or []
        ...
        for r in range(world_size):
            ...
            try:
                ...
                print(
                    f"[GPU {fabric.global_rank}] Warning: failed to unpickle rank {r} payload: {e}"
                )

        if verbose_memory:
            print(
                f"[GPU {fabric.global_rank}] Successfully gathered {len(all_chunks)} total chunks"
            )
...
def evaluate_overlapping_chunks_fast(
    ...
    print(
        f"[GPU {fabric.global_rank}] Moving {len(songs)} songs to GPU and pre-normalizing..."
    )
    for sid in songs:
    ...
    cand_cliques = torch.tensor(
        [d["clique_id"] for _, d in song_list], device=fabric.device
    )
    cand_versions = torch.tensor(
        [d["version_id"] for _, d in song_list], device=fabric.device
    )
    ...
    end_idx = (
        ...
        else start_idx + per_gpu
    )
    local_queries = song_list[start_idx:end_idx]

    print(
        f"[GPU {fabric.global_rank}] Processing {len(local_queries)} queries vs {total_songs} candidates"
    )
    ...
    for i in pbar:
        ...
        for j, (c_sid, c_data) in enumerate(song_list):
            ...
            if topk == 1:
                ...
                dists[j] = (
                    torch.topk(dist_flat, k, largest=False)[0].mean()
                    if k > 0
                    else dist_flat.mean()
                )
        ...
        pbar.set_postfix(
            {"AP": f"{mean_ap:.4f}", "last": f"{cur_ap:.4f}", "s/q": f"{avg_t:.2f}"}
        )
        ...
        if checkpoint_manager.enabled and (i + 1) % 100 == 0:
            checkpoint_manager.save_evaluation_checkpoint(
                i, partial, local_queries, args
            )
    ...
    if partial:
        ...
        if fabric.is_global_zero:
            print(
                f"[GPU {fabric.global_rank}] Local results: "
...
def gather_results_safely(fabric: Fabric, *results: torch.Tensor) -> List[torch.Tensor]:
    """
    ...
    size0 = torch.tensor(
        [local_tensors[0].numel()], device=fabric.device, dtype=torch.long
    )
    ...
    for idx, res in enumerate(local_tensors):
        ...
        all_padded = safe_all_gather_with_timeout(
            fabric, res_padded, timeout_seconds=300
        )

        if all_padded is None:
            print(
                f"[GPU {fabric.global_rank}] Gather failed for result {idx}, returning local only"
            )
        ...
        gathered = (
            torch.cat(pieces, dim=0).detach().cpu() if pieces else torch.tensor([])
        )
...
def print_results(
    aps: torch.Tensor, r1s: torch.Tensor, rpcs: torch.Tensor, args: Any, fabric: Fabric
) -> None:
    """
